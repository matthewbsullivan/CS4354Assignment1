...
public class  Main {
    ...
    /**
     * A menu for a movie store inventory. Users can add, remove, or view a movie's entry
     * in the inventory, as well as view a list of the inventory's contents.
     * @param args: takes input from keyboard for Scanner
     */
    public static void main(String[] args) {
        Inventory inv = new Inventory();
        inv.loadFromFile(); //load a saved ArrayList<Movie> to inv from file "Inventory"
        Movie mov = new Movie();
        String input;
        ...
        do {
            ...
            switch (input) {
                ...
                case "2":
                    ...
                    inv.removeMovie(removalCandidate); //checks Inventory for a SKU match and removes the matching movie if found
                    s.nextLine();
                    break;
                case "3":
                    System.out.println("Enter SKU of movie to display: ");
                    int displayCandidate = s.nextInt();
                    inv.displayMovie(displayCandidate); //checks Inventory for a SKU match and displays the matching movie if found
                    s.nextLine();
                    break;
                case "4":
                    inv.displayInventory(); //displays entire contents of Inventory
                    break;
                case "5":
